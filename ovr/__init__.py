...
Bool = ctypes.c_char    #< Boolean type
...
class PoseStatef(ctypes.Structure):
    ...
    _fields_ = [
        ...
        ("pad0", ctypes.c_char * 4),       #< \internal struct pad.
        ("TimeInSeconds", ctypes.c_double),          #< Absolute time of this state sample.
    ]
...
class FovPort(ctypes.Structure):
    ...
    _pack_ = 4
    _fields_ = [
        ("UpTan", ctypes.c_float),     #< The tangent of the angle between the viewing vector and the top edge of the field of view.
        ("DownTan", ctypes.c_float),   #< The tangent of the angle between the viewing vector and the bottom edge of the field of view.
        ("LeftTan", ctypes.c_float),   #< The tangent of the angle between the viewing vector and the left edge of the field of view.
        ("RightTan", ctypes.c_float),  #< The tangent of the angle between the viewing vector and the right edge of the field of view.
    ]
...
class HmdDesc(ctypes.Structure):
    ...
    _fields_ = [
        ...
        # skipping 64-bit only padding... # ("pad0", ctypes.c_char * 4)),    #< \internal struct paddding.
        ("ProductName", ctypes.c_char * 64),               #< UTF8-encoded product identification string (e.g. "Oculus Rift DK1").
        ("Manufacturer", ctypes.c_char * 64),              #< UTF8-encoded HMD manufacturer identification string.
        ("VendorId", ctypes.c_short),                      #< HID (USB) vendor identifier of the device.
        ("ProductId", ctypes.c_short),                     #< HID (USB) product identifier of the device.
        ("SerialNumber", ctypes.c_char * 24),              #< Sensor (and display) serial number.
        ("FirmwareMajor", ctypes.c_short),                 #< Sensor firmware major version.
        ("FirmwareMinor", ctypes.c_short),                 #< Sensor firmware minor version.
        ("CameraFrustumHFovInRadians", ctypes.c_float),    #< External tracking camera frustum horizontal field-of-view (if present).
        ("CameraFrustumVFovInRadians", ctypes.c_float),    #< External tracking camera frustum vertical field-of-view (if present).
        ("CameraFrustumNearZInMeters", ctypes.c_float),    #< External tracking camera frustum near Z (if present).
        ("CameraFrustumFarZInMeters", ctypes.c_float),     #< External tracking camera frustum far Z (if present).
        ("AvailableHmdCaps", ctypes.c_uint),              #< Capability bits described by ovrHmdCaps which the HMD currently supports.
        ("DefaultHmdCaps", ctypes.c_uint),                #< Capability bits described by ovrHmdCaps which are default for the current Hmd.
        ("AvailableTrackingCaps", ctypes.c_uint),         #< Capability bits described by ovrTrackingCaps which the system currently supports.
        ("DefaultTrackingCaps", ctypes.c_uint),           #< Capability bits described by ovrTrackingCaps which are default for the current system.
        ...
        ("DisplayRefreshRate", ctypes.c_float),            #< Nominal refresh rate of the display in cycles per second at the time of HMD creation.
        # skipping 64-bit only padding... # ("pad1", ctypes.c_char * 4)),    #< \internal struct paddding.
    ]
...
class SensorData(ctypes.Structure):
    ...
    _fields_ = [
        ...
        ("Temperature", ctypes.c_float),       #< Temperature of the sensor in degrees Celsius.
        ("TimeInSeconds", ctypes.c_float),     #< Time when the reported IMU reading took place in seconds. \see ovr_GetTimeInSeconds
    ]
...
class TrackingState(ctypes.Structure):
    ...
    _fields_ = [
        ...
        ("LastCameraFrameCounter", ctypes.c_uint32), 
        ("pad0", ctypes.c_char * 4),  #< \internal struct padding
    ]
...
class FrameTiming(ctypes.Structure):
    ...
    _fields_ = [
        ...
        ("DisplayMidpointSeconds", ctypes.c_double), 
        ...
        ("FrameIntervalSeconds", ctypes.c_double), 
        # Application frame index for which we requested timing.
        ("AppFrameIndex", ctypes.c_uint), 
        ...
        ("DisplayFrameIndex", ctypes.c_uint), 
    ]
...
class Texture(ctypes.Structure):
    ...
    _fields_ = [
        ...
        # skipping 64-bit only padding... # ("pad0", ctypes.c_char * 4)),     #< \internal struct padding
...
class InitParams(ctypes.Structure):
    ...
    _fields_ = [
        ...
        # skipping 64-bit only padding... # ("pad0", ctypes.c_char * 4)),  #< \internal
    ]
...
libovr.ovr_Initialize.argtypes = [ctypes.POINTER(InitParams)]
def initialize(params):
    ...
    \see ovr_Shutdown
    """
    result = libovr.ovr_Initialize(None if params is None else byref(params))
    if FAILURE(result):
...
libovr.ovr_GetLastErrorInfo.argtypes = [ctypes.POINTER(ErrorInfo)]
def getLastErrorInfo():
    ...
    """
    errorInfo = ErrorInfo()
    result = libovr.ovr_GetLastErrorInfo(None if errorInfo is None else byref(errorInfo))
    return errorInfo


# Translated from header file OVR_CAPI_0_7_0.h line 1011
libovr.ovr_GetVersionString.restype = ctypes.POINTER(ctypes.c_char)
...
libovr.ovr_TraceMessage.restype = ctypes.c_int
libovr.ovr_TraceMessage.argtypes = [ctypes.c_int, ctypes.POINTER(ctypes.c_char)]
def traceMessage(level, message):
    ...
    """
    result = libovr.ovr_TraceMessage(level, None if message is None else byref(message))
    return result
...
libovr.ovr_Create.argtypes = [ctypes.POINTER(Hmd), ctypes.POINTER(GraphicsLuid)]
def create():
    """
...
libovr.ovr_GetEnabledCaps.restype = ctypes.c_uint
...
libovr.ovr_SetEnabledCaps.argtypes = [Hmd, ctypes.c_uint]
...
libovr.ovr_ConfigureTracking.argtypes = [Hmd, ctypes.c_uint, ctypes.c_uint]
...
libovr.ovr_GetTrackingState.argtypes = [Hmd, ctypes.c_double]
...
libovr.ovr_GetInputState.argtypes = [Hmd, ctypes.c_uint, ctypes.POINTER(InputState)]
def getInputState(hmd, controllerTypeMask):
    ...
    result = libovr.ovr_GetInputState(hmd, controllerTypeMask, None if inputState is None else byref(inputState))
    if FAILURE(result):
...
libovr.ovr_SetControllerVibration.argtypes = [Hmd, ctypes.c_uint, ctypes.c_float, ctypes.c_float]
...
libovr.ovr_DestroySwapTextureSet.argtypes = [Hmd, ctypes.POINTER(SwapTextureSet)]
...
libovr.ovr_DestroyMirrorTexture.argtypes = [Hmd, ctypes.POINTER(Texture)]
def destroyMirrorTexture(hmd, mirrorTexture):
    ...
    """
    result = libovr.ovr_DestroyMirrorTexture(hmd, None if mirrorTexture is None else byref(mirrorTexture))
    return result


# Translated from header file OVR_CAPI_0_7_0.h line 1473
libovr.ovr_GetFovTextureSize.restype = Sizei
libovr.ovr_GetFovTextureSize.argtypes = [Hmd, EyeType, FovPort, ctypes.c_float]
...
libovr.ovr_SubmitFrame.argtypes = [Hmd, ctypes.c_uint, ctypes.POINTER(ViewScaleDesc), ctypes.POINTER(ctypes.POINTER(LayerHeader)), ctypes.c_uint]
def submitFrame(hmd, frameIndex, viewScaleDesc, layerPtrList, layerCount):
    ...
    """
    layerPtrList = ctypes.POINTER(LayerHeader)(layerPtrList)
    result = libovr.ovr_SubmitFrame(hmd, frameIndex, None if viewScaleDesc is None else byref(viewScaleDesc), None if layerPtrList is None else byref(layerPtrList), layerCount)
    if FAILURE(result):
...
libovr.ovr_GetFrameTiming.argtypes = [Hmd, ctypes.c_uint]
...
libovr.ovr_GetTimeInSeconds.restype = ctypes.c_double
...
libovr.ovr_CreateSwapTextureSetGL.argtypes = [Hmd, GLuint, ctypes.c_int, ctypes.c_int, ctypes.POINTER(ctypes.POINTER(SwapTextureSet))]
def createSwapTextureSetGL(hmd, format, width, height):
    ...
    result = libovr.ovr_CreateSwapTextureSetGL(hmd, format, width, height, None if outTextureSet is None else byref(outTextureSet))
    if FAILURE(result):
...
libovr.ovrTimewarpProjectionDesc_FromProjection.argtypes = [Matrix4f, ctypes.c_uint]
...
libovr.ovrMatrix4f_OrthoSubProjection.argtypes = [Matrix4f, Vector2f, ctypes.c_float, ctypes.c_float]
...
libovr.ovr_GetEyePoses.argtypes = [Hmd, ctypes.c_uint, Vector3f * 2, Posef * 2, ctypes.POINTER(TrackingState)]
def getEyePoses(hmd, frameIndex, hmdToEyeViewOffset, outEyePoses):
    ...
    result = libovr.ovr_GetEyePoses(hmd, frameIndex, hmdToEyeViewOffset, outEyePoses, None if outHmdTrackingState is None else byref(outHmdTrackingState))
